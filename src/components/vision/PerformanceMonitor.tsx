...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  // Simulate performance monitoring (in real implementation, this would come from actual metrics)
  useEffect(() => {
    if (!isVisible) return;
    ...
    const interval = setInterval(() => {
      ...
      const mockMetrics = {
        ...
        frameRate: 10 + Math.random() * 20,
        processingTime: 20 + Math.random() * 100,
        batteryLevel: 80 + Math.random() * 20,
        thermalState: ['nominal', 'fair', 'serious'][Math.floor(Math.random() * 3)]
      };

      setPerformanceData(mockMetrics);

      // Apply optimizations
      const optimizations = optimizer.optimizePerformance(mockMetrics);
  ...
  }, [isVisible, onOptimizationChange]);
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2 bg-black/70 text-white px-3 py-2 rounded-lg text-xs">
        ...
        <span className={getPerformanceColor(performanceData.cpuUsage)}>
          CPU: {Math.round(performanceData.cpuUsage)}%
        </span>
        <span className={getPerformanceColor(performanceData.frameRate, true)}>
          FPS: {Math.round(performanceData.frameRate)}
        </span>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={getPerformanceColor(performanceData.cpuUsage)}>
                {Math.round(performanceData.cpuUsage)}%
              </span>
            </div>
            <Progress 
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={getPerformanceColor(performanceData.memoryUsage)}>
                {Math.round(performanceData.memoryUsage)}%
              </span>
            </div>
            <Progress 
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={getPerformanceColor(performanceData.frameRate, true)}>
                {Math.round(performanceData.frameRate)}
              </span>
            </div>
            <Progress 
              value={(performanceData.frameRate / 30) * 100} 
              className="h-1"
            />
          </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={getPerformanceColor(performanceData.processingTime)}>
                {Math.round(performanceData.processingTime)}ms
              </span>
            </div>
            <Progress 
              value={Math.min((performanceData.processingTime / 200) * 100, 100)} 
              className="h-1"
            />
          </div>
        </div>
        ...
        {optimizationStatus.recommendations.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {optimizationStatus.recommendations.slice(0, 2).map((rec, index) => (
