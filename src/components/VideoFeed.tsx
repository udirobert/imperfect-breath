...
const VideoFeed = ({ videoRef, isActive, landmarks, trackingStatus }: VideoFeedProps) => {
  ...
  const statusStyles: { [key in TrackingStatus]: { border: string; message: string; showIcon: boolean } } = {
    IDLE: { border: 'border-white', message: '', showIcon: false },
    INITIALIZING: { border: 'border-yellow-400', message: 'Initializing...', showIcon: true },
    TRACKING: { border: 'border-green-400', message: '', showIcon: false },
    NO_FACE: { border: 'border-red-500', message: 'Face not detected. Adjust lighting or position.', showIcon: false },
  };

  const { border, message, showIcon } = statusStyles[trackingStatus] || statusStyles.IDLE;
    ...
    <div className={cn(
      ...
      {message && (
        <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-2">
          ...
          <p className="text-white text-xs font-semibold text-center">{message}</p>
        </div>
      )}
      <div className={cn("absolute inset-0 bg-black/20", { 'hidden': !!message })} />
