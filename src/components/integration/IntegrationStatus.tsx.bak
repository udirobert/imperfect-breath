...
export const IntegrationStatus: React.FC = () => {
  ...
  const { health, hasVisionService, hasFlowService, hasLensService } = useSystemHealth();
  ...
  useEffect(() => {
    const updateIntegrations = () => {
      const items: IntegrationItem[] = [
        ...
        // AI Analysis
        {
          id: 'ai',
          name: 'AI Analysis',
          description: 'Intelligent breathing pattern analysis',
          status: health.ai ? 'connected' : 'error',
          required: true,
          setupUrl: '/ai-settings',
          details: health.ai 
    ...
    updateIntegrations();
  }, [isAuthenticated, hasWallet, health, hasVisionService, hasFlowService, hasLensService, user]);
  ...
  const getStatusBadge = (status: IntegrationItem['status'], required: boolean) => {
    switch (status) {
      case 'connected':
        return <Badge variant="success">Connected</Badge>;
      case 'disconnected':
        return <Badge variant={required ? "destructive" : "secondary"}>Disconnected</Badge>;
      case 'error':
  ...
  const overallHealth = requiredConnected === requiredCount ? 'good' : 
                       requiredConnected > 0 ? 'partial' : 'poor';
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Badge 
              variant={overallHealth === 'good' ? 'success' : 
                      overallHealth === 'partial' ? 'warning' : 'destructive'}
            >
      ...
      <CardContent>
        ...
        <div className="mt-6 p-4 bg-muted rounded-lg">
          ...
          <ul className="text-sm space-y-1 text-muted-foreground">
            <li>• <strong>Essential:</strong> Sign in and configure AI settings</li>
            <li>• <strong>Enhanced:</strong> Connect wallet for blockchain features</li>
            <li>• <strong>Advanced:</strong> Set up vision service for real-time analysis</li>
          </ul>
        </div>
      </CardContent>
    </Card>
