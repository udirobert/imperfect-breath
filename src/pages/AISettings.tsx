...
import { CheckCircle, XCircle, ExternalLink, Eye, EyeOff, TestTube, Loader2 } from 'lucide-react';
...
const AISettings = () => {
  ...
  useEffect(() => {
    ...
    AI_PROVIDERS.forEach(provider => {
      const key = AIConfigManager.getApiKey(provider.id);
      if (key) {
  ...
  const saveApiKey = (providerId: string) => {
    ...
    if (key && key.trim()) {
      AIConfigManager.setApiKey(providerId, key.trim());
      toast.success(`API key saved for ${AI_PROVIDERS.find(p => p.id === providerId)?.name}`);
    } else {
      AIConfigManager.removeApiKey(providerId);
      toast.success(`API key removed for ${AI_PROVIDERS.find(p => p.id === providerId)?.name}`);
    }
  };
  ...
  const testApiKey = async (providerId: string) => {
    ...
    try {
      ...
      if (result && !result.error) {
        toast.success(`${AI_PROVIDERS.find(p => p.id === providerId)?.name} API key is working!`);
      } else {
  ...
  const maskApiKey = (key: string) => {
    ...
    return key.substring(0, 4) + '•'.repeat(Math.max(key.length - 8, 0)) + key.substring(key.length - 4);
  };

  const isKeyConfigured = (providerId: string) => {
    return !!AIConfigManager.getApiKey(providerId);
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          Configure AI providers to get personalized insights on your breathing sessions
        </p>
      </div>
        ...
        <TabsContent value="providers" className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={!apiKeys[provider.id]?.trim() || testing[provider.id]}
                      >
                        {testing[provider.id] ? (
                    ...
                    {isKeyConfigured(provider.id) && !showKeys[provider.id] && (
                      <p className="text-xs text-muted-foreground">
                        Current key: {maskApiKey(AIConfigManager.getApiKey(provider.id) || '')}
                      </p>
                    )}
                  </div>
                  ...
                  <Alert>
                    <AlertDescription className="text-sm">
                      ...
                      They are only used to communicate directly with {provider.name}.
                    </AlertDescription>
                  </Alert>
                </CardContent>
              </Card>
          ...
          <div className="flex justify-between items-center">
            <div className="text-sm text-muted-foreground">
              Configured providers: {AI_PROVIDERS.filter(p => isKeyConfigured(p.id)).length}/{AI_PROVIDERS.length}
            </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="prose prose-sm max-w-none">
                <p>
                  Our AI analysis feature provides personalized insights on your breathing sessions by analyzing:
                </p>
                <ul className="list-disc list-inside space-y-1">
                  <li><strong>Breath Hold Performance:</strong> How long you can comfortably hold your breath</li>
                  <li><strong>Stillness & Focus:</strong> Your restlessness score during meditation</li>
                  <li><strong>Session Patterns:</strong> Which breathing techniques work best for you</li>
                  <li><strong>Progress Tracking:</strong> How you're improving over time</li>
                </ul>
                ...
                <div className="grid gap-3 mt-2">
                  <div className="p-3 border rounded-lg">
                    <strong>OpenAI GPT-4:</strong> Advanced reasoning and detailed explanations
                  </div>
                  <div className="p-3 border rounded-lg">
                    <strong>Anthropic Claude:</strong> Thoughtful, safety-focused insights
                  </div>
                  <div className="p-3 border rounded-lg">
                    <strong>Google Gemini:</strong> Fast analysis with multimodal understanding
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3 text-sm">
                <Alert>
                  <AlertDescription>
                    <strong>Your data is private:</strong> Session data is only sent to the AI providers you configure.
