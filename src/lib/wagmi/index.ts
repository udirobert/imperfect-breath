...
const trackWalletError = (errorType, errorDetails) => {
  if (typeof window !== 'undefined' && window.__walletState) {
...
export const safeWallet = {
  ...
  isAvailable: () => {
    ...
    if (window.walletApi && typeof window.walletApi.isAvailable === 'function') {
      return window.walletApi.isAvailable();
    }
    
    // Then check original safe wallet
    if (window.safeWallet && typeof window.safeWallet.isAvailable === 'function') {
  ...
  request: async (method, params) => {
    ...
    if (window.walletApi && typeof window.walletApi.request === 'function') {
      try {
        ...
        console.warn('Error using production walletApi, trying alternatives:', err);
