 ...
 */
export async function getTimeline(address: string): Promise<{ items: SocialPost[] }> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || `Error ${response.status}: Failed to fetch timeline`);
    }
...
export async function getTrendingPatterns(): Promise<TrendingPattern[]> {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.message || `Error ${response.status}: Failed to fetch trending patterns`);
    }
 ...
 */
export async function reactToPost(publicationId: string, remove: boolean): Promise<boolean> {
  try {
    // Get auth token from local storage
    const token = localStorage.getItem('lens_auth_token');
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.message || `Error ${response.status}: Failed to react to post`);
    }
 ...
 */
export async function followAccount(address: string): Promise<{ success: boolean, error?: string }> {
  try {
    // Get auth token from local storage
    const token = localStorage.getItem('lens_auth_token');
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.message || `Error ${response.status}: Failed to follow account`);
    }
  ...
  try {
    ...
    const response = await fetch('/api/social/share', {
      ...
      body: JSON.stringify({
        ...
        content: sessionData.content || `Just completed a ${sessionData.patternName} breathing session with ${sessionData.score}% focus!`,
        metadata: {
          breathingPattern: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.message || `Error ${response.status}: Failed to share session`);
    }
